...
class UploadReservationCSV(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            return {'error': 'An error occurred while processing data {str(e)}'}, 500
...
reservation_model = api.model('Reservation', {
    ...
    'inv_desc': fields.String(required=True, description='Description of the inventory'),
...
class CreateReservation(Resource):
    ...
    def post(self):
        ...
        if 'data' in inventory_data and len(inventory_data['data']) > 0:
            ...
            print("First Inventory Item:", first_inventory_item)
            inv_id = first_inventory_item['inv_id']
            inv_name = first_inventory_item['inv_name']
            inv_description = first_inventory_item.get('inv_description','')
        else:
        ...
        new_reservation = {
            ...
            'inv_name': inv_name,  # Use the extracted inventory data
            'inv_id': inv_id,
...
class CreateReservation(Resource):
    ...
    def post(self):
        ...
        if 'data' in inventory_data and len(inventory_data['data']) > 0:
            ...
            inv_name_set = set(item['inv_name'] for item in inventory_data['data'])
        else:
            abort(500, error='no inventory data available')
        ...
        if inv_name not in inv_name_set:
            abort(400, error=f'inv_name {inv_name} does not exist in the inventory')

         # Retrieve inv_description from the inventory data
        inv_description = None
        for item in inventory_data['data']:
            if item['inv_id'] == inv_id:
                inv_desc = item.get('inv_desc', '')
                break
...
if response.status_code == 200:
    ...
    if 'data' in inventory_data:
        ...
        for item in inventory_items:
            ...
            inv_desc = item.get('inv_desc', '')  # Use get() to handle optional fields
